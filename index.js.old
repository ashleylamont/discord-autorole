...
discordClient.once("ready", () => {
    ...
    setInterval(function () {
        ...
        discordClient.users.forEach((val) => {
            let user = val; //Because I can't be arsed to rewrite my code.
            if (!user.bot && user.presence.game !== null && user.presence.game.name !== null && user.presence.game.name !== "" && user.presence.game.type === 0) {
                postgresClient.query(`SELECT * FROM users WHERE userid=$1 AND gamename=$2; `, [user.id, user.presence.game.name.toLowerCase()]).then(res => {

                    if (res.rowCount === 0) {
                        postgresClient.query(`INSERT INTO users (userid,gamename) VALUES ($1,$2);`, [user.id, user.presence.game.name.toLowerCase()]).then(res => {

                            if (verbose) {
                                console.log(`Added ${user.presence.game.name} to ${user.username}`)
                            }
                            ...
                            console.log(`Presence: ${user.presence.game},${user.presence.game.name}`)
                        });
                    }
        ...
        discordClient.guilds.forEach((val) => {
            ...
            postgresClient.query(`SELECT * FROM rolebindings WHERE serverid=$1`, [guild.id]).then(res => {
                ...
                res.rows.forEach((val) => {
                    ...
                    users.forEach((val) => {
                        let guildMember = val;
                        postgresClient.query(`SELECT * FROM users WHERE gamename=$1 and userid=$2`, [rolebinding.gamename, guildMember.user.id]).then(res => {

                            if (res.rowCount > 0) {
                                if (guildMember.roles.has(rolebinding.roleid) === false) {
...
discordClient.on('message', message => {
    ...
    if (!(!message.content.startsWith(prefix) || message.author.bot) && allowedChannels.includes(message.channel.type)) {
        ...
        if (command === "set-role") {
            ...
            postgresClient.query(`SELECT * FROM users WHERE userid=$1`, [userid.toString()]).then(res => {

                if (res.rows.length === 0) {
                    message.reply("I'm sorry, I don't appear to have that user in my database.")
                } else {
                    let games = "User has played ";
                    res.rows.forEach((val, index) => {
                        if (index < res.rows.length - 1) {
                            games = games + val.gamename + ", ";
                        } else {
                            games = games + val.gamename + ".";
                        }
