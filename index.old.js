...
discordClient.once("ready", () => {
    ...
    setInterval(function () {
        ...
        discordClient.guilds.forEach((val) => {
            ...
            postgresClient.query(`SELECT * FROM serverconfig WHERE serverid=$1`, [guild.id]).then(res => {

                if (res.rowCount === 0) {
                    postgresClient.query(`INSERT INTO serverconfig (serverid,language) VALUES ($1,$2)`, [guild.id, "en"]).then(res => {
            ...
            postgresClient.query(`SELECT * FROM rolebindings WHERE serverid=$1`, [guild.id]).then(res => {
                ...
                res.rows.forEach((val) => {
                    ...
                    users.forEach((val) => {
                        ...
                        if (!user.bot && guildMember.presence.activity !== null && guildMember.presence.activity.type === 0) {
...
function sendMessage(message, context, key, params) {
    ...
    if (nonEnglishServers.includes(serverid)) {
        postgresClient.query(`SELECT * FROM serverconfig WHERE serverid=$1`, [serverid]).then(res => {
        ...
        if (command === "set-role") {
            ...
            if (args[0] === undefined) {
                ...
                if (i18next.languages.includes(args[0])) {
                    postgresClient.query(`UPDATE serverconfig SET language=$1 WHERE serverid=$2`, [args[0], message.guild.id]).then(res => {
